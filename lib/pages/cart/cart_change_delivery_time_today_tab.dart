...
class PageToday extends StatefulWidget {
  ...
  String selectedHour;
  Function function;
  PageToday({Key? key, required this.selectedDate, required this.selectedHour, required this.function}) : super(key: key);

  @override
  State<PageToday> createState() => _PageTodayState();
}

class _PageTodayState extends State<PageToday> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    if (getTodayString() == widget.selectedDate) {
        ...
        children: [
          ...
          (hourNow >= 21 && hourNow < 24)
              ? SizedBox(
                  width: width,
                  height: height * 0.75,
                  child: const Center(child: Text('Bugün için uygun teslimat saati mevcut değil!')),
                )
              : const SizedBox.shrink()
        ],
      ),
